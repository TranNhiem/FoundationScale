"""Tests that the stated online objective-member set and the OnlineObjective
protocol are the same set.

``online._OBJECTIVE_MEMBERS`` restates, by hand, the members the protocol
demands. It is stated rather than read from ``OnlineObjective``'s
...
WHAT THIS MODULE PROVES: every member named by ``_OBJECTIVE_MEMBERS`` is
load-bearing for ``OnlineObjective``, and ``_absent_members`` names exactly
the members that an object does not expose.

WHAT THIS MODULE DOES NOT PROVE: that a protocol-satisfying object computes
an online objective, that its signatures or arithmetic are correct, or that
runtime member presence discriminates an online objective from a preference
objective.
...
* a member the constant states and the protocol does NOT demand makes the
  ...
  the drop-one leg;
* a shipped ``OnlineDPOLoss`` satisfies both ``OnlineObjective`` and
  ``PreferenceObjective`` -- the measurement behind online.py's
  WHAT IS NOT CLAIMED paragraph about the two protocols demanding the same
  members.
"""
...
from foundationscale.rl.online_objectives import OnlineDPOLoss
...
def test_stub_exposing_exactly_the_stated_members_satisfies_the_protocol() -> None:
    ...
    assert isinstance(_objective_stub(), OnlineObjective)
...
def test_dropping_any_one_stated_member_breaks_the_protocol_check() -> None:
    ...
    still_satisfying = tuple(
        ...
        if isinstance(_objective_stub(omit=name), OnlineObjective)
    )
    assert still_satisfying == (), (
        f"{len(still_satisfying)} of {len(_OBJECTIVE_MEMBERS)} stated members "
        f"are not demanded by OnlineObjective: {still_satisfying!r}"
    )
...
def test_a_shipped_online_objective_also_satisfies_the_preference_protocol() -> None:
    # This is the MEASUREMENT behind OnlineObjective's WHAT IS NOT CLAIMED
    # paragraph: the online and preference protocols demand the same members,
    # so runtime member presence cannot discriminate one family from the
    # other. The shipped loss drives the measurement rather than a double.
    objective = OnlineDPOLoss()

    assert isinstance(objective, OnlineObjective)
    assert isinstance(objective, PreferenceObjective)
