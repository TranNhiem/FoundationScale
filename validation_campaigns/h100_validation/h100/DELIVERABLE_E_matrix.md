...
files are now wired in and the plane builds green at 43 stages. **How many other stages are in
...
|---|---|---|
| stages green | **43/43** | full run; a red stage aborts the build. The number is derived by `gate_doc_stage_count.py` from the build script's own `STAGES` array on every build (#194) -- a denominator that is retyped by hand is a denominator that lags. See the note below on why this cell now carries only the number. |
