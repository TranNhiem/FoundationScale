...
files are now wired in and the plane builds green at 36 stages. **How many other stages are in
...
|---|---|---|
| stages green | **36/36** | full run, red stage aborts the build. Restated 2026-09-01 from a stale `17/17`: the count is now derived by `gate_doc_stage_count.py` from the build script's own `STAGES` array on every build, because a denominator that is retyped by hand is a denominator that lags (#194). |
