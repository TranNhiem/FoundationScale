...
Tier 0 blocks tier 1 because a tier-1 row that toggles an undeclared axis measures nothing attributably: the run would train on the engine default either way, and no artifact could distinguish "the knob worked" from "the knob was never wired." Rows T1-9 through T1-14 each named the tier-0 row they waited on. #373 shipped all of those, so each of the six now names what released it: T1-9 through T1-12 have since RUN on a tray -- T1-9 and T1-10 GREEN, T1-12 UNMEASURED on its missing flash_attn arm, T1-11 still returning the #408 false RED -- while T1-13 and T1-14 are REFUSED — the dependency shipped as a refusal, so their run arms have nothing to execute.
...
| ID | Claim | Run arm | Control arm (must differ) | Cost | Status today |
|---|---|---|---|---|---|
| T1-9 | adamw vs fused vs adafactor vs sgd differ in step time and loss curve | 4 arms, same seed | identical curves across optimizers = the knob did nothing | ~15 min x4 | MEASURED GREEN (#405/#406) on a GB200 tray: 4 optimizer arms (adamw, adamw_torch_fused, sgd, adafactor), 40 STEP-LABELLED loss points read from each arm's run_manifest.json; max abs delta 5.046e-01 > tol 1.0e-06, so the declared optimizer demonstrably moves the trajectory. Earned only after the #406 telemetry widening put a per-step train_loss_curve in the manifest and the reader was made step-keyed -- the pre-fix reader compared ONE aggregate per arm, so the same GREEN was unearned. |
| T1-10 | accum=4,bs=1 and accum=1,bs=4 give the SAME loss curve | both arms | if they differ beyond tolerance, accumulation is wrong | ~15 min x2 | MEASURED GREEN (#405/#406) on a GB200 tray: accum4_bs1 vs accum1_bs4 agree over the 40-step INTERSECTION of the two arms' step labels; worst gap 0.0133574 at accum1_bs4 step 7 (2.9078900814056396 vs 2.9212474822998047). Keyed on step, not on list position -- two arms logging at different cadences hold different steps at the same index, and a positional zip would have compared step-against-wrong-step and called it a verdict. |
