...
| T1-21 | an image-bearing corpus trains and the vision tower's weights MOVE | image arm | a text-only arm through the same path: tower weights must NOT move | ~30 min | MEASURED on a GB200 tray (#448, follow-up to #371): the declared-image arm is PROCESSOR-FAMILY DEPENDENT, and which families it serves had never been measured. train_image_collator_or_refuse builds one text per feature but a FLAT image list across ALL features, and passes the raw row text through with no chat template and no image placeholder token. Against a Qwen2.5-VL processor the arm is FUNCTIONAL -- pixel_values present for all three image-list shapes. Against a Gemma-4 processor it is UNREACHABLE at any batch size, for three independent causes that must ALL be fixed: (1) a flat list normalises to ONE image-batch, so N images and N texts read as 1 vs N; (2) with no placeholder token the pixels have nowhere to land; (3) TOKENIZE_MAX_LENGTH 128 is smaller than the prompt surface's OWN recorded 258 tokens per image, so truncation halves the expanded image block -- a measured internal contradiction, not a transformers quirk. The two-row pixel-drop probe catches it before a step is paid for, and the escaping ValueError is adjudicated RED 5 by the #380 boundary handler rather than exiting 1. Controls that HELD: an image column declared over a text-only corpus refuses 96 (#410), and a model with no processor refuses 96 rather than downgrading to a tokenizer. The row's own claim is still UNMEASURED: there is no adjudicator, and on the Gemma-4 family the arm cannot reach a step for the tower weights to move in. |
