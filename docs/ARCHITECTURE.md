...
The RL plane is 27 modules; `src/foundationscale/rl/` measures 19426 lines and
...
`src/` is 44876 LOC across 57 files; `tests/` adds 67839 `.py` LOC (its
conftest carries the skip guard). Beside the package:

| Tree | Contents |
|---|---|
| `tools/` | 9979 Python LOC of CLIs over the package: `emit_run_manifest`, `live_save_gate`, `real_checkpoint_probe`, `preflight/`, `mutate`, `census`. |
...
Repo-wide: 232647 git-tracked `.py`/`.sh`/`.md` lines.
