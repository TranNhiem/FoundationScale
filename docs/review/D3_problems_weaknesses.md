...
**What it is.** FoundationScale presents as a foundation-model training framework. Most of the measured content of `src/foundationscale` (48,001 LOC) is a verification plane: gates, checkpoint readers, a parity comparator, topology validation, provenance manifests. The training-primitive probe is no longer zero: axis A is small but nonzero while delegation markers dominate.
...
- **`F2_docs#6`** (PARTLY_TRUE, corrected): the finding claimed no troubleshooting page; in fact one *does* exist, at `validation_campaigns/h100_validation/h100/LAUNCH.md`, scoped to the H100 launch plane. What is absent is one scoped to the *package*.
