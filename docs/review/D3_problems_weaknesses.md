...
**What it is.** FoundationScale presents as a foundation-model training framework. The measured content of `src/foundationscale` (16,213 LOC) is exclusively a verification plane: gates, checkpoint readers, a parity comparator, topology validation, provenance manifests. The census training-code probe is unambiguous:
