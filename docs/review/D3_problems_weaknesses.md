...
**What it is.** FoundationScale presents as a foundation-model training framework. Most of the measured content of `src/foundationscale` (41,687 LOC) is a verification plane: gates, checkpoint readers, a parity comparator, topology validation, provenance manifests. The training-primitive probe reads zero on every marker:
