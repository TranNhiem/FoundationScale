...
flowchart TB
  ...
  PKG["[installed foundationscale package]<br/>37,169 LOC / 49 files<br/>verification plane + delegating train/ (1,305 LOC)<br/>0 training primitives, step loop rented from transformers.Trainer"]
...
```mermaid
flowchart TB
  TESTS["tests/<br/>109 Python files / 48,668 LOC"]
  TOOLS["tools/<br/>31 Python files / 9,533 LOC"]
  SRC["src/ as importer<br/>49 Python files / 37,169 LOC"]

  FS["src/foundationscale<br/>49 Python files / 37,169 LOC<br/>root __init__.py exports nothing"]
  ...
  TESTS -->|"231 Python import statements"| FS
  TOOLS -->|"14 Python import statements"| FS
  SRC -->|"129 Python import statements, source not disaggregated"| FS
