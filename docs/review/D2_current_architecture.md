...
flowchart TB
  ...
  PKG["[installed foundationscale package]<br/>28,534 LOC / 37 files<br/>verification plane + delegating train/ (1,305 LOC)<br/>0 training primitives, step loop rented from transformers.Trainer"]
...
```mermaid
flowchart TB
  TESTS["tests/<br/>85 Python files / 40,871 LOC"]
  TOOLS["tools/<br/>31 Python files / 9,533 LOC"]
  SRC["src/ as importer<br/>37 Python files / 28,534 LOC"]

  FS["src/foundationscale<br/>37 Python files / 28,534 LOC<br/>root __init__.py exports nothing"]
  ...
  TESTS -->|"157 Python import statements"| FS
  TOOLS -->|"14 Python import statements"| FS
  SRC -->|"69 Python import statements, source not disaggregated"| FS
