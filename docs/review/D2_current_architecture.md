...
flowchart TB
  ...
  PKG["[installed foundationscale package]<br/>38,201 LOC / 50 files<br/>verification plane + delegating train/ (1,305 LOC)<br/>0 training primitives, step loop rented from transformers.Trainer"]
...
- The operational plane is shell and tool-heavy: `launchers/` contains 11,602 shell LOC plus 1,615 Python LOC, while `tools/` contains 9,533 Python LOC. `validation_campaigns/h100_validation/` adds another 34,167 Python LOC and 6,489 shell LOC.
...
```mermaid
flowchart TB
  TESTS["tests/<br/>119 Python files / 52,444 LOC"]
  TOOLS["tools/<br/>31 Python files / 9,533 LOC"]
  SRC["src/ as importer<br/>50 Python files / 38,201 LOC"]

  FS["src/foundationscale<br/>50 Python files / 38,201 LOC<br/>root __init__.py exports nothing"]
  ...
  TESTS -->|"247 Python import statements"| FS
  TOOLS -->|"14 Python import statements"| FS
  SRC -->|"130 Python import statements, source not disaggregated"| FS
...
| Launch orchestration | 11,602 shell LOC in `launchers/`; preflight is scoped to one Gemma-4-E4B / GB200-tray launch; census denominator control is hardwired to one launcher | **ESTATE-ONLY, not a package launcher API** |
...
| Hardware validation plane | `validation_campaigns/h100_validation/` contributes 34,167 Python LOC and 6,489 shell LOC | **CODE PRESENT, estate-scoped; it is not an installed trainer** |
