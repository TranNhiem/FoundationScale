...
flowchart TB
  ...
  PKG["[installed foundationscale package]<br/>47,180 LOC / 60 files<br/>verification plane + delegating train/ (4,724 LOC)<br/>few owned primitives (RL backward/step, 2 collectives); main step loop rented from transformers.Trainer"]
...
- The operational plane is shell and tool-heavy: `launchers/` contains 13,008 shell LOC plus 1,615 Python LOC, while `tools/` contains 9,997 Python LOC. `validation_campaigns/h100_validation/` adds another 34,169 Python LOC and 6,706 shell LOC.
...
```mermaid
flowchart TB
  TESTS["tests/<br/>172 Python files / 73,000 LOC"]
  TOOLS["tools/<br/>31 Python files / 9,997 LOC"]
  SRC["src/ as importer<br/>60 Python files / 47,180 LOC"]

  FS["src/foundationscale<br/>60 Python files / 47,180 LOC<br/>root __init__.py exports nothing"]
