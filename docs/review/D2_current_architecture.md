...
flowchart TB
  ...
  PKG["[installed foundationscale package]<br/>37,655 LOC / 50 files<br/>verification plane + delegating train/ (1,305 LOC)<br/>0 training primitives, step loop rented from transformers.Trainer"]
...
- The operational plane is shell and tool-heavy: `launchers/` contains 11,115 shell LOC plus 1,615 Python LOC, while `tools/` contains 9,533 Python LOC. `validation_campaigns/h100_validation/` adds another 34,167 Python LOC and 6,489 shell LOC.
...
```mermaid
flowchart TB
  TESTS["tests/<br/>111 Python files / 49,452 LOC"]
  TOOLS["tools/<br/>31 Python files / 9,533 LOC"]
  SRC["src/ as importer<br/>50 Python files / 37,655 LOC"]

  FS["src/foundationscale<br/>50 Python files / 37,655 LOC<br/>root __init__.py exports nothing"]
  ...
  TESTS -->|"236 Python import statements"| FS
...
| Launch orchestration | 11,115 shell LOC in `launchers/`; preflight is scoped to one Gemma-4-E4B / GB200-tray launch; census denominator control is hardwired to one launcher | **ESTATE-ONLY, not a package launcher API** |
