...
flowchart TB
  ...
  PKG["[installed foundationscale package]<br/>21,534 LOC / 31 files<br/>verification plane + delegating train/ (1,305 LOC)<br/>0 training primitives, step loop rented from transformers.Trainer"]
...
- The operational plane is shell and tool-heavy: `launchers/` contains 10,716 shell LOC plus 1,615 Python LOC, while `tools/` contains 9,514 Python LOC. `validation_campaigns/h100_validation/` adds another 33,385 Python LOC and 6,414 shell LOC.
...
```mermaid
flowchart TB
  TESTS["tests/<br/>73 Python files / 32,221 LOC"]
  TOOLS["tools/<br/>31 Python files / 9,514 LOC"]
  SRC["src/ as importer<br/>31 Python files / 21,534 LOC"]

  FS["src/foundationscale<br/>31 Python files / 21,534 LOC<br/>root __init__.py exports nothing"]
  ...
  TESTS -->|"114 Python import statements"| FS
  TOOLS -->|"14 Python import statements"| FS
  SRC -->|"39 Python import statements, source not disaggregated"| FS
