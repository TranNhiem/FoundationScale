...
flowchart TB
  ...
  PKG["[installed foundationscale package]<br/>29,755 LOC / 39 files<br/>verification plane + delegating train/ (1,305 LOC)<br/>0 training primitives, step loop rented from transformers.Trainer"]
...
```mermaid
flowchart TB
  TESTS["tests/<br/>89 Python files / 42,372 LOC"]
  TOOLS["tools/<br/>31 Python files / 9,533 LOC"]
  SRC["src/ as importer<br/>39 Python files / 29,755 LOC"]

  FS["src/foundationscale<br/>39 Python files / 29,755 LOC<br/>root __init__.py exports nothing"]
  ...
  TESTS -->|"179 Python import statements"| FS
  TOOLS -->|"14 Python import statements"| FS
  SRC -->|"80 Python import statements, source not disaggregated"| FS
