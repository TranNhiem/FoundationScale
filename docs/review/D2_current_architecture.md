...
flowchart TB
  ...
  PKG["[installed foundationscale package]<br/>40,850 LOC / 52 files<br/>verification plane + delegating train/ (1,305 LOC)<br/>0 training primitives, step loop rented from transformers.Trainer"]
...
- The operational plane is shell and tool-heavy: `launchers/` contains 12,636 shell LOC plus 1,615 Python LOC, while `tools/` contains 9,904 Python LOC. `validation_campaigns/h100_validation/` adds another 34,169 Python LOC and 6,706 shell LOC.
...
```mermaid
flowchart TB
  TESTS["tests/<br/>132 Python files / 59,616 LOC"]
  TOOLS["tools/<br/>31 Python files / 9,904 LOC"]
  SRC["src/ as importer<br/>52 Python files / 40,850 LOC"]

  FS["src/foundationscale<br/>52 Python files / 40,850 LOC<br/>root __init__.py exports nothing"]
  ...
  TESTS -->|"283 Python import statements"| FS
...
| Launch orchestration | 12,636 shell LOC in `launchers/`; preflight is scoped to one Gemma-4-E4B / GB200-tray launch; census denominator control is hardwired to one launcher | **ESTATE-ONLY, not a package launcher API** |
