...
flowchart TB
  ...
  PKG["[installed foundationscale package]<br/>39,084 LOC / 51 files<br/>verification plane + delegating train/ (1,305 LOC)<br/>0 training primitives, step loop rented from transformers.Trainer"]
...
- The operational plane is shell and tool-heavy: `launchers/` contains 12,024 shell LOC plus 1,615 Python LOC, while `tools/` contains 9,594 Python LOC. `validation_campaigns/h100_validation/` adds another 34,169 Python LOC and 6,706 shell LOC.
...
```mermaid
flowchart TB
  TESTS["tests/<br/>122 Python files / 54,903 LOC"]
  TOOLS["tools/<br/>31 Python files / 9,594 LOC"]
  SRC["src/ as importer<br/>51 Python files / 39,084 LOC"]

  FS["src/foundationscale<br/>51 Python files / 39,084 LOC<br/>root __init__.py exports nothing"]
  ...
  TESTS -->|"260 Python import statements"| FS
  TOOLS -->|"14 Python import statements"| FS
  SRC -->|"133 Python import statements, source not disaggregated"| FS
...
| Launch orchestration | 12,024 shell LOC in `launchers/`; preflight is scoped to one Gemma-4-E4B / GB200-tray launch; census denominator control is hardwired to one launcher | **ESTATE-ONLY, not a package launcher API** |
