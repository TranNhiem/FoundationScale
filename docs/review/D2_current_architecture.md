...
flowchart TB
  ...
  PKG["[installed foundationscale package]<br/>44,246 LOC / 54 files<br/>verification plane + delegating train/ (4,724 LOC)<br/>few owned primitives (RL backward/step, 2 collectives); main step loop rented from transformers.Trainer"]
...
- The operational plane is shell and tool-heavy: `launchers/` contains 13,008 shell LOC plus 1,615 Python LOC, while `tools/` contains 9,972 Python LOC. `validation_campaigns/h100_validation/` adds another 34,169 Python LOC and 6,706 shell LOC.
...
```mermaid
flowchart TB
  TESTS["tests/<br/>154 Python files / 67,036 LOC"]
  TOOLS["tools/<br/>31 Python files / 9,972 LOC"]
  SRC["src/ as importer<br/>54 Python files / 44,246 LOC"]

  FS["src/foundationscale<br/>54 Python files / 44,246 LOC<br/>root __init__.py exports nothing"]
  ...
  TESTS -->|"320 Python import statements"| FS
  TOOLS -->|"14 Python import statements"| FS
  SRC -->|"146 Python import statements, source not disaggregated"| FS
