...
- The operational plane is shell and tool-heavy: `launchers/` contains 10,263 shell LOC plus 1,615 Python LOC, while `tools/` contains 9,514 Python LOC. `validation_campaigns/h100_validation/` adds another 31,313 Python LOC and 4,996 shell LOC.
...
| Launch orchestration | 10,263 shell LOC in `launchers/`; preflight is scoped to one Gemma-4-E4B / GB200-tray launch; census denominator control is hardwired to one launcher | **ESTATE-ONLY, not a package launcher API** |
...
| Hardware validation plane | `validation_campaigns/h100_validation/` contributes 31,313 Python LOC and 4,996 shell LOC | **CODE PRESENT, estate-scoped; it is not an installed trainer** |
