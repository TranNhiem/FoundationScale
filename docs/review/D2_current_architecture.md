...
flowchart TB
  ...
  PKG["[installed foundationscale package]<br/>48,001 LOC / 60 files<br/>verification plane + delegating train/ (4,724 LOC)<br/>few owned primitives (RL backward/step, 2 collectives); main step loop rented from transformers.Trainer"]
...
```mermaid
flowchart TB
  TESTS["tests/<br/>180 Python files / 74,666 LOC"]
  TOOLS["tools/<br/>32 Python files / 10,183 LOC"]
  SRC["src/ as importer<br/>60 Python files / 48,001 LOC"]

  FS["src/foundationscale<br/>60 Python files / 48,001 LOC<br/>root __init__.py exports nothing"]
  ...
  TESTS -->|"368 Python import statements"| FS
  TOOLS -->|"14 Python import statements"| FS
  SRC -->|"161 Python import statements, source not disaggregated"| FS
