...
flowchart TB
  ...
  PKG["[installed foundationscale package]<br/>47,269 LOC / 60 files<br/>verification plane + delegating train/ (4,724 LOC)<br/>few owned primitives (RL backward/step, 2 collectives); main step loop rented from transformers.Trainer"]
...
- The operational plane is shell and tool-heavy: `launchers/` contains 13,008 shell LOC plus 1,615 Python LOC, while `tools/` contains 10,177 Python LOC. `validation_campaigns/h100_validation/` adds another 34,169 Python LOC and 6,706 shell LOC.
...
```mermaid
flowchart TB
  TESTS["tests/<br/>173 Python files / 73,234 LOC"]
  TOOLS["tools/<br/>32 Python files / 10,177 LOC"]
  SRC["src/ as importer<br/>60 Python files / 47,269 LOC"]

  FS["src/foundationscale<br/>60 Python files / 47,269 LOC<br/>root __init__.py exports nothing"]
  ...
  TESTS -->|"356 Python import statements"| FS
  TOOLS -->|"14 Python import statements"| FS
  SRC -->|"157 Python import statements, source not disaggregated"| FS
...
| Launch orchestration | 13,008 shell LOC in `launchers/`; preflight is scoped to one Gemma-4-E4B / GB200-tray launch; census denominator control is hardwired to one launcher | **ESTATE-ONLY, not a package launcher API** |
