...
flowchart TB
  ...
  PKG["[installed foundationscale package]<br/>50,006 LOC / 62 files<br/>verification plane + delegating train/ (4,724 LOC)<br/>few owned primitives (RL backward/step, 2 collectives); main step loop rented from transformers.Trainer"]
...
- The operational plane is shell and tool-heavy: `launchers/` contains 13,008 shell LOC plus 1,615 Python LOC, while `tools/` contains 10,183 Python LOC. `validation_campaigns/h100_validation/` adds another 34,169 Python LOC and 6,706 shell LOC.
...
```mermaid
flowchart TB
  TESTS["tests/<br/>184 Python files / 76,650 LOC"]
  TOOLS["tools/<br/>32 Python files / 10,183 LOC"]
  SRC["src/ as importer<br/>62 Python files / 50,006 LOC"]

  FS["src/foundationscale<br/>62 Python files / 50,006 LOC<br/>root __init__.py exports nothing"]
  ...
  TESTS -->|"384 Python import statements"| FS
  TOOLS -->|"14 Python import statements"| FS
  SRC -->|"175 Python import statements, source not disaggregated"| FS
