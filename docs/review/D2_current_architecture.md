...
flowchart TB
  ...
  PKG["[installed foundationscale package]<br/>22,790 LOC / 33 files<br/>verification plane + delegating train/ (1,305 LOC)<br/>0 training primitives, step loop rented from transformers.Trainer"]
...
- The operational plane is shell and tool-heavy: `launchers/` contains 10,716 shell LOC plus 1,615 Python LOC, while `tools/` contains 9,533 Python LOC. `validation_campaigns/h100_validation/` adds another 33,385 Python LOC and 6,414 shell LOC.
...
```mermaid
flowchart TB
  TESTS["tests/<br/>75 Python files / 33,569 LOC"]
  TOOLS["tools/<br/>31 Python files / 9,533 LOC"]
  SRC["src/ as importer<br/>33 Python files / 22,790 LOC"]

  FS["src/foundationscale<br/>33 Python files / 22,790 LOC<br/>root __init__.py exports nothing"]
  ...
  TESTS -->|"122 Python import statements"| FS
  TOOLS -->|"14 Python import statements"| FS
  SRC -->|"46 Python import statements, source not disaggregated"| FS
...
| Launch orchestration | 10,716 shell LOC in `launchers/`; preflight is scoped to one Gemma-4-E4B / GB200-tray launch; census denominator control is hardwired to one launcher | **ESTATE-ONLY, not a package launcher API** |
