...
flowchart TB
  ...
  PKG["[installed foundationscale package]<br/>40,014 LOC / 52 files<br/>verification plane + delegating train/ (1,305 LOC)<br/>0 training primitives, step loop rented from transformers.Trainer"]
...
- The operational plane is shell and tool-heavy: `launchers/` contains 12,319 shell LOC plus 1,615 Python LOC, while `tools/` contains 9,594 Python LOC. `validation_campaigns/h100_validation/` adds another 34,169 Python LOC and 6,706 shell LOC.
...
```mermaid
flowchart TB
  TESTS["tests/<br/>124 Python files / 55,866 LOC"]
  TOOLS["tools/<br/>31 Python files / 9,594 LOC"]
  SRC["src/ as importer<br/>52 Python files / 40,014 LOC"]

  FS["src/foundationscale<br/>52 Python files / 40,014 LOC<br/>root __init__.py exports nothing"]
  ...
  TESTS -->|"268 Python import statements"| FS
  TOOLS -->|"14 Python import statements"| FS
  SRC -->|"142 Python import statements, source not disaggregated"| FS
...
| Launch orchestration | 12,319 shell LOC in `launchers/`; preflight is scoped to one Gemma-4-E4B / GB200-tray launch; census denominator control is hardwired to one launcher | **ESTATE-ONLY, not a package launcher API** |
