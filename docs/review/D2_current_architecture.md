...
flowchart TB
  ...
  PKG["[installed foundationscale package]<br/>35,565 LOC / 47 files<br/>verification plane + delegating train/ (1,305 LOC)<br/>0 training primitives, step loop rented from transformers.Trainer"]
...
- The operational plane is shell and tool-heavy: `launchers/` contains 10,864 shell LOC plus 1,615 Python LOC, while `tools/` contains 9,533 Python LOC. `validation_campaigns/h100_validation/` adds another 33,385 Python LOC and 6,414 shell LOC.
...
```mermaid
flowchart TB
  TESTS["tests/<br/>101 Python files / 47,199 LOC"]
  TOOLS["tools/<br/>31 Python files / 9,533 LOC"]
  SRC["src/ as importer<br/>47 Python files / 35,565 LOC"]

  FS["src/foundationscale<br/>47 Python files / 35,565 LOC<br/>root __init__.py exports nothing"]
  ...
  TESTS -->|"217 Python import statements"| FS
  TOOLS -->|"14 Python import statements"| FS
  SRC -->|"121 Python import statements, source not disaggregated"| FS
...
| Launch orchestration | 10,864 shell LOC in `launchers/`; preflight is scoped to one Gemma-4-E4B / GB200-tray launch; census denominator control is hardwired to one launcher | **ESTATE-ONLY, not a package launcher API** |
...
| Hardware validation plane | `validation_campaigns/h100_validation/` contributes 33,385 Python LOC and 6,414 shell LOC | **CODE PRESENT, estate-scoped; it is not an installed trainer** |
