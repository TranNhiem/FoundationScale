...
flowchart TB
  ...
  PKG["[installed foundationscale package]<br/>46,475 LOC / 60 files<br/>verification plane + delegating train/ (4,724 LOC)<br/>few owned primitives (RL backward/step, 2 collectives); main step loop rented from transformers.Trainer"]
...
```mermaid
flowchart TB
  TESTS["tests/<br/>170 Python files / 71,188 LOC"]
  TOOLS["tools/<br/>31 Python files / 9,997 LOC"]
  SRC["src/ as importer<br/>60 Python files / 46,475 LOC"]

  FS["src/foundationscale<br/>60 Python files / 46,475 LOC<br/>root __init__.py exports nothing"]
  ...
  TESTS -->|"350 Python import statements"| FS
