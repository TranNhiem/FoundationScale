...
flowchart TB
  ...
  PKG["[installed foundationscale package]<br/>50,094 LOC / 62 files<br/>verification plane + delegating train/ (4,724 LOC)<br/>few owned primitives (RL backward/step, 2 collectives); main step loop rented from transformers.Trainer"]
...
```mermaid
flowchart TB
  TESTS["tests/<br/>190 Python files / 78,483 LOC"]
  TOOLS["tools/<br/>32 Python files / 10,183 LOC"]
  SRC["src/ as importer<br/>62 Python files / 50,094 LOC"]

  FS["src/foundationscale<br/>62 Python files / 50,094 LOC<br/>root __init__.py exports nothing"]
  ...
  TESTS -->|"401 Python import statements"| FS
