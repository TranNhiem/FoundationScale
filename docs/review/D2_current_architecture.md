...
flowchart TB
  ...
  PKG["[installed foundationscale package]<br/>41,858 LOC / 52 files<br/>verification plane + delegating train/ (1,305 LOC)<br/>0 training primitives, step loop rented from transformers.Trainer"]
...
```mermaid
flowchart TB
  TESTS["tests/<br/>139 Python files / 62,714 LOC"]
  TOOLS["tools/<br/>31 Python files / 9,972 LOC"]
  SRC["src/ as importer<br/>52 Python files / 41,858 LOC"]

  FS["src/foundationscale<br/>52 Python files / 41,858 LOC<br/>root __init__.py exports nothing"]
  ...
  TESTS -->|"294 Python import statements"| FS
  TOOLS -->|"14 Python import statements"| FS
  SRC -->|"144 Python import statements, source not disaggregated"| FS
