...
**The load-bearing fact, restated once — on two axes, because one axis lies.** `src/foundationscale/` measures 46345 lines and implements a small number of training *primitives*: across all 52 git-tracked `src/*.py`, `nn.Module` subclasses 0, dataloader 0, forward pass 0, but backward 1 (`src/foundationscale/rl/trainer.py:752`), optimizer step 3 (`src/foundationscale/rl/trainer.py:753` among them), distributed collective 2 (`src/foundationscale/train/loop.py:1151`, `:1214`), and `torch` imported at *module* scope 2 — both inside `if TYPE_CHECKING:` blocks that never run (#510); it is imported at function scope in 6 files (`checkpoint/dcp.py`, `rl/prompt_surface.py`, `rl/torch_backend.py`, `rl/trainer.py`, `train/loop.py`, `verify/parity.py`). The package still ships a *delegating* main trainer — `src/foundationscale/train/`, 4,724 LOC across four files, which constructs a `transformers.Trainer` and calls `trainer.train()` — with axis B markers dominant (1 trainer, 4 fit/train/save calls, 3 `AutoModel.from_pretrained`, 1 collator, 30 lazy imports). Reporting only either axis is finding #223/#245: primitive markers cannot distinguish delegation from not training, and now they also show a small owned RL core. The proposals below are sized to the real gap — not "build a trainer" but *backend-neutralise the one that exists*, wire the verification plane into more than its single save seam, isolate model specifics, and extend verification to the load path.
...
flowchart TB
  ...
  subgraph ESTATE["Estate layer (kept, edges only)"]
    LAU["launchers/*.sh (13008 LOC, 7 files)"]
    H100["validation_campaigns/h100_validation/ (34169 .py LOC, 66 files)"]
...
### D5-2. Thin, model-agnostic training path
  ...
  **Current design** — the package implements a small number of training *primitives*: across all 52 git-tracked `src/*.py` an AST probe finds no `nn.Module` subclass, no `DataLoader`, and no `forward`, but 1 `backward()` (`src/foundationscale/rl/trainer.py:752`), 3 `optimizer.step()` calls (`src/foundationscale/rl/trainer.py:753` among them), 2 `dist.all_reduce` collectives (`src/foundationscale/train/loop.py:1151`, `:1214`), and 2 module-scope torch imports. It still ships a *delegating* main entry point — `src/foundationscale/train/` (`loop.py` 4,134 LOC, `cli.py` 505, `__main__.py` 55, `__init__.py` 30) builds a `transformers.Trainer` and calls `trainer.train()`. Both statements are true at once, and stating only either axis is the #223/#245 error: primitive markers cannot tell delegation from not training, and delegation markers cannot erase the owned RL backward/step core. What is genuinely absent is a *backend-neutral* trainer seam — the shipped main path is one `transformers` backend, single-node causal-LM SFT, covered at 62% (#228), and it is not the Megatron/NeMo path the launchers drive. No DCP checkpoint writer exists anywhere in the package — readers only.
...
### D5-5. A declared public API surface
  ...
  **Why it is a problem** — The test suite — `tests/` (70298 .py LOC, 168 files; 347 import statements), the package's largest measured consumer — pins private names, so every refactor fights its own tests (T2#2).
...
- **The bash launchers** — `launchers/*.sh` (13008 LOC, 7 files) — and **the H100 validation harness** — `validation_campaigns/h100_validation/` (34169 .py LOC, 66 files): preserved per the standing H100/GB200 constraint. Flagged honestly: D4's Keep here rests on census plus constraint, with zero direct findings; launcher internals and the harness are UNMEASURED by this review, not validated. Wiring the harness to `GateHooks` is future work after D5-1, explicitly out of this architecture's scope.
- **`validation_campaigns/h100_validation/*.sh` (6706 LOC, 5 files)**: same disposition — estate plane, kept at the edges.
