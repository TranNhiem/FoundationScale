...
flowchart TB
  ...
  subgraph ESTATE["Estate layer (kept, edges only)"]
    LAU["launchers/*.sh (11221 LOC, 7 files)"]
...
- **The bash launchers** — `launchers/*.sh` (11221 LOC, 7 files) — and **the H100 validation harness** — `validation_campaigns/h100_validation/` (34167 .py LOC, 66 files): preserved per the standing H100/GB200 constraint. Flagged honestly: D4's Keep here rests on census plus constraint, with zero direct findings; launcher internals and the harness are UNMEASURED by this review, not validated. Wiring the harness to `GateHooks` is future work after D5-1, explicitly out of this architecture's scope.
