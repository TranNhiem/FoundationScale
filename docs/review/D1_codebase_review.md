...
The census records 241087 git-tracked .py/.sh/.md lines repo-wide. The trees, from largest to smallest:
...
- **`tests/`** — `tests/` (73237 .py LOC, 173 files; 356 import statements) in the census's wording. Test file names are not in the evidence slice, so this review cannot enumerate what the suite covers by file; section 9 reasons from what is visible elsewhere.
...
- The test suite (73237 .py LOC) over `tests/` (173 tracked test files in the census's wording) — the individual test module names are not in this slice, so per-area coverage inside `tests/` is **unmeasured** from here; enumerating it requires a file listing of `tests/`.
