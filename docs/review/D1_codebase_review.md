...
- **`validation_campaigns/h100_validation/`** — an off-package validation-and-repair plane for an H100 estate launch. `validation_campaigns/h100_validation/` (32408 .py LOC, 64 files) plus `validation_campaigns/h100_validation/*.sh` (5186 LOC, 4 files). It is the single largest tree in the repository, bigger than the installable package. It contains a large family of `patch_*.py` scripts, a second family of `gate_*.py` scripts, executor `apply_*.py` scripts, a generated subtree at `validation_campaigns/h100_validation/h100/gen/`, deliverable documents under `validation_campaigns/h100_validation/h100/` (architecture review, validation report, matrix, EVIDENCE.md, LAUNCH.md), and two pytest files.
