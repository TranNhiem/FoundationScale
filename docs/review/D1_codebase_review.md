...
The census records 237615 git-tracked .py/.sh/.md lines repo-wide. The trees, from largest to smallest:
...
`src/foundationscale/` measures 46475 lines in the census's wording and is organised into six subpackages plus two top-level modules:
...
- The test suite (71188 .py LOC) over `tests/` (170 tracked test files in the census's wording) — the individual test module names are not in this slice, so per-area coverage inside `tests/` is **unmeasured** from here; enumerating it requires a file listing of `tests/`.
