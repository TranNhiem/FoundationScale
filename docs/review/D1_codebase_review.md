...
The census records 268428 git-tracked .py/.sh/.md lines repo-wide. The trees, from largest to smallest:

- **`validation_campaigns/h100_validation/`** — an off-package validation-and-repair plane for an H100 estate launch. `validation_campaigns/h100_validation/` (34169 .py LOC, 66 files) plus `validation_campaigns/h100_validation/*.sh` (6706 LOC, 5 files). It is the single largest tree in the repository, bigger than the installable package. It contains a large family of `patch_*.py` scripts, a second family of `gate_*.py` scripts, executor `apply_*.py` scripts, a generated subtree at `validation_campaigns/h100_validation/h100/gen/`, deliverable documents under `validation_campaigns/h100_validation/h100/` (architecture review, validation report, matrix, EVIDENCE.md, LAUNCH.md), and two pytest files.
- **`tests/`** — `tests/` (78485 .py LOC, 190 files; 401 import statements) in the census's wording. Test file names are not in the evidence slice, so this review cannot enumerate what the suite covers by file; section 9 reasons from what is visible elsewhere.
- **`src/`** — the installable package. `src/` = 50094 LOC across 62 files. Five files exceed 2000 lines: `src/foundationscale/train/loop.py` (4,134), `src/foundationscale/provenance/manifest.py` (2,777), `src/foundationscale/gates/adjudication.py` (2,585), `src/foundationscale/gates/checkpoint_gates.py` (2,182), and `src/foundationscale/rl/policy_gradient.py` (2,134). (Until #509 this line wrapped the whole phrase in one code span, which the countables anchor could not match, so it sat at a stale `18761 LOC across 25 files` while the gate reported every `src_loc` site agreeing.)
- **`launchers/`** — `launchers/` contains 13008 shell LOC plus 1615 Python LOC, according to the census. The two largest files are `launchers/test_launcher_contracts.sh` (4381) and `launchers/launch_g4e4b_lora_1tray.sh` (1913). Notably, `launchers/__pycache__/` with two `.pyc` files appears in this listing; the pycache directory also appears in the census's exclusion list, so those artifacts do not count in any anchored number, but their presence in the tree listing is a hygiene smell.
- **`tools/`** — script-level adjudicators. `tools/` contains 10183 Python LOC. Dominated by the `tools/preflight/` package (4184 across 23 modules); `tools/mutate.py` (1745) is the largest single file.
...
`src/foundationscale/` measures 50094 lines in the census's wording and is organised into six subpackages plus two top-level modules:
...
3. **Launcher shell environment** — the real parameterisation for production runs; `validation_campaigns/h100_validation/estate.env.example` is the template of record for the H100 estate.
...
- The test suite (78485 .py LOC) over `tests/` (190 tracked test files in the census's wording) — the individual test module names are not in this slice, so per-area coverage inside `tests/` is **unmeasured** from here; enumerating it requires a file listing of `tests/`.

NOT covered, with evidence for the absence or the gap:

- The `validation_campaigns/h100_validation/patch_*.py` family: files the tree shows. Their only test presence visible in the slice is incidental. Patching scripts of several hundred lines apiece with no tests is the largest untested mass in the repository.
...
- **The largest tree is not the product**: `validation_campaigns/h100_validation/` outweighs `src/`. It is an estate-renovation workshop, not user-facing code, and most files there are one-time patches.
...
- **`.pyc` files in the tracked tree listing for `launchers/` and `validation_campaigns/h100_validation/`**: excluded from all census numbers but present where they shouldn't be; a newcomer will wonder which tree is authoritative.
