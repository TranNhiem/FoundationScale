...
The census records 190786 git-tracked .py/.sh/.md lines repo-wide. The trees, from largest to smallest:
...
- **`tests/`** — `tests/` (54903 .py LOC, 122 files; 260 import statements) in the census's wording. Test file names are not in the evidence slice, so this review cannot enumerate what the suite covers by file; section 9 reasons from what is visible elsewhere.
...
`src/foundationscale/` measures 39084 lines in the census's wording and is organised into six subpackages plus two top-level modules:
...
- The test suite (54903 .py LOC) over `tests/` (122 tracked test files in the census's wording) — the individual test module names are not in this slice, so per-area coverage inside `tests/` is **unmeasured** from here; enumerating it requires a file listing of `tests/`.
