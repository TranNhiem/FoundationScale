...
- **`validation_campaigns/h100_validation/`** — an off-package validation-and-repair plane for an H100 estate launch. `validation_campaigns/h100_validation/` (31313 .py LOC, 63 files) plus `validation_campaigns/h100_validation/*.sh` (4986 LOC, 4 files). It is the single largest tree in the repository, bigger than the installable package. It contains a large family of `patch_*.py` scripts, a second family of `gate_*.py` scripts, executor `apply_*.py` scripts, a generated subtree at `validation_campaigns/h100_validation/h100/gen/`, deliverable documents under `validation_campaigns/h100_validation/h100/` (architecture review, validation report, matrix, EVIDENCE.md, LAUNCH.md), and two pytest files.
- **`tests/`** — `tests/` (29578 .py LOC, 65 files; 99 import statements) in the census's wording. Test file names are not in the evidence slice, so this review cannot enumerate what the suite covers by file; section 9 reasons from what is visible elsewhere.
...
`src/foundationscale/` measures 18915 lines in the census's wording and is organised into six subpackages plus two top-level modules:
...
- The test suite (29578 .py LOC) over `tests/` (65 tracked test files in the census's wording) — the individual test module names are not in this slice, so per-area coverage inside `tests/` is **unmeasured** from here; enumerating it requires a file listing of `tests/`.
