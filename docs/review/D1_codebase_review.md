...
The census records 232647 git-tracked .py/.sh/.md lines repo-wide. The trees, from largest to smallest:
...
- **`tests/`** — `tests/` (67839 .py LOC, 157 files; 325 import statements) in the census's wording. Test file names are not in the evidence slice, so this review cannot enumerate what the suite covers by file; section 9 reasons from what is visible elsewhere.
- **`src/`** — the installable package. `src/` = 44876 LOC across 57 files. Five files exceed 2000 lines: `src/foundationscale/train/loop.py` (4,134), `src/foundationscale/provenance/manifest.py` (2,777), `src/foundationscale/gates/adjudication.py` (2,585), `src/foundationscale/gates/checkpoint_gates.py` (2,182), and `src/foundationscale/rl/policy_gradient.py` (2,134). (Until #509 this line wrapped the whole phrase in one code span, which the countables anchor could not match, so it sat at a stale `18761 LOC across 25 files` while the gate reported every `src_loc` site agreeing.)
...
- **`tools/`** — script-level adjudicators. `tools/` contains 9979 Python LOC. Dominated by the `tools/preflight/` package (4184 across 23 modules); `tools/mutate.py` (1745) is the largest single file.
...
`src/foundationscale/` measures 44876 lines in the census's wording and is organised into six subpackages plus two top-level modules:
...
- The test suite (67839 .py LOC) over `tests/` (157 tracked test files in the census's wording) — the individual test module names are not in this slice, so per-area coverage inside `tests/` is **unmeasured** from here; enumerating it requires a file listing of `tests/`.
