...
- **`tests/`** — `tests/` (29572 .py LOC, 65 files; 99 import statements) in the census's wording. Test file names are not in the evidence slice, so this review cannot enumerate what the suite covers by file; section 9 reasons from what is visible elsewhere.
