...
`src/` = 46261 LOC across 60 files. `launchers/` contains 10231 shell LOC plus 1615 Python
LOC, and `validation_campaigns/h100_validation/` adds another 34169 Python LOC and 6706 shell LOC on top of the
package. `tools/` contains 9997 Python LOC. 236202 git-tracked .py/.sh/.md lines repo-wide.

```
src/foundationscale/   the package: gates/, checkpoint/, verify/, provenance/,
                       topology.py, models/, train/, integrate.py
tests/                 the test suite (69992 .py LOC); conftest carries the skip guard
...
* **Mutation testing asks the other question.** The mutation corpus is 106 rows over 15
  modules. Of those, 91 are MUST_FIRE mutants and 15 are MUST_PASS controls. Exit codes
